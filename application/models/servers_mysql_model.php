...
class Servers_mysql_model extends CI_Model{
    ...
    function get_total_record_master(){
        $this->db->select('m.id, m.host, m.port, m.tags');
        $this->db->from('db_servers_mysql m ');
        $this->db->join('mysql_replication repl', 'repl.host=m.host and repl.port=m.port and repl.is_master="1"', 'inner');
        $this->db->order_by("m.host", 'desc');
        $query = $this->db->get();
        if ($query->num_rows() > 0)
	{
	    return $query->result_array();
