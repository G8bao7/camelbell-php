...
FROM (SELECT * FROM os_status_history WHERE create_time>=ADDDATE(NOW(),INTERVAL -30 MINUTE) order BY create_time DESC) his
...
JOIN db_servers_os os ON his.ip=os.host 
WHERE his.create_time>=ADDDATE(NOW(),INTERVAL -30 MINUTE)
GROUP BY os.tags
...
ORDER BY io_writes DESC limit 10;")->result_array();
    ...
    function ajax_get_alarm(){
         $data_array = $this->db->query("select * from  alarm_history WHERE create_time>=ADDDATE(NOW(),INTERVAL -6 Hour)  order by create_time desc limit 12;")->result_array();
