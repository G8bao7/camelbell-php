...
class Servers_mysql extends Front_Controller {
    ...
    public function add(){
		...
		$this->form_validation->set_rules('replicate_ip',  'lang:replicate_ip', 'trim|required');
		/*
		...
		*/
		...
		{
		    $replicate_ip = ($this->input->post('replicate_ip') == '-') ? $this->input->post('host') : $this->input->post('replicate_ip');
		    $data['error_code']=0;
		    $data = array(
			...
			'password'=>'-',
		        'tags'=>$this->input->post('tags'),
                        ...
                        'monitor_os'=>$this->input->post('monitor_os'),
		    ...
		    $osData = array(
				'host'=>$this->input->post('host'),
				'tags'=>$this->input->post('tags'),
				'monitor'=>$this->input->post('monitor_os'),
			    );
		    $this->servers_os->insOrUpd($osData);
	    ...
	    */
	    ...
	    else
	    {
		    ...
		    /* ++ guoqi
		     get old record
		    */
		    $old_record = $this->servers->get_record_by_id($id);
		    ...
		    /* ++ guoqi
		     if new host|port then delete db_status
		    */
		    $srcHost = $old_record['host'];
		    $srcPort = $old_record['port'];
		    $newHost = $data['host'];
		    $newPort = $data['port'];
		    
		    if("$srcHost" != "$newHost" or $srcPort != $newPort){
			    $this->servers->db_status_remove($id);
		    }
		    redirect(site_url('servers_mysql/index'));
	    }
	...
	if(isset($_POST['submit']) && $_POST['submit']=='batch_add')
		...
		$replicate_ip = $this->input->post('replicate_ip_'.$n);
		$replicate_ip = ($this->input->post('replicate_ip_'.$n) == '-') ? $this->input->post('host_'.$n) : $this->input->post('replicate_ip_'.$n);
		$username = '-';
		$password = '-';
			...
			$data = array(
			    ...
			    'slow_query'=>$this->input->post('slow_query_'.$n),
