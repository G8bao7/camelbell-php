...
function get_replication_tree($array,$host='---',$port='---',$level=0,$host_replicate='---'){
    $repeat='&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
	$str_repeat = '';
    if($level) {
		for($j = 0; $j < $level; $j ++) {
    ...
    foreach ( ( array ) $array as $v ) {
        if(($v['master_server']==$host or $v['master_server']==$host_replicate ) and $v['master_port']==$port)
        {
            $host_v=$v['host'];
            $host_replicate_v=$v['replicate_ip'];
            $port_v=$v['port'];
            $v['host']=$str_repeat.$v['host'];
            $v['level']=$level;
            $newarray[] = $v;
            $temparray = get_replication_tree($array,$host_v,$port_v,$level+1, $host_replicate_v);
...
function check_dbtype($data){  
    if($data =="mysql"){
        ...
        $db_type_img="<img src='images/none.png' />";
    }
    return  $db_type_img;
}
...
function check_metadata($data){
    if($data == 'OFF'){
        $data_c = "<span class='badge badge-success'>OFF</span>";
    }
    else{
        $data_c = "<span class='badge badge-important'>ON</span>";
    }
    return $data_c;
}

function check_sync_binlog($data){
    if($data == 1){
        $data_c="<span class='label label-warning'>".$data."</span>";
    }
    else if($data > 100){
        $data_c="<span class='label label-success'>".$data."</span>";
    }
    else{
	$data_c=$data;
    }
...
function check_trx_commit($data){
    if($data == 2){
        $data_c="<span class='label label-success'>".$data."</span>";
    }
    else if($data == 1){
        ...
        $data_c="<span class='label label-important'>".$data."</span>";
    }
    return $data_c;
}

function check_pct($data, $data_total, $min_pct){
    $cur_pct = round($data/$data_total, 2) * 100;
    if($cur_pct > $min_pct){
        $data_c="<span class='label label-warning'>".$data."</span>";
    }
    else{
